...
def test_trace_store_writes_scenario_and_runtime_metadata(tmp_path):
    store = TraceStore.create(
        tmp_path,
        session_id="sess_store",
        adapter="debugpy",
        scenario="验证 odd branch",
        cwd="/repo",
        command=["python", "app.py"],
    )

    session = load_session_metadata(store.session_path)

    assert session["scenario"] == "验证 odd branch"
...
def test_dump_graph_json_and_render_mermaid_with_repeated_edge(tmp_path):
    ...
    graph = build_line_flow_graph([first, second, first, second, first, second], loop_threshold=3)
